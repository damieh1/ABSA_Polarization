...
Aspect spans and their dependencies were extracted using `biaffine_dep_parser.py`, based on the `SuPar library` Biaffine dependency parser (English TreeBanks). Output is aligned to the sentiment classification task and formatted using `spitter_jsonl.py`.
